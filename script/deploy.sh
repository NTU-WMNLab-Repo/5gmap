...
deploy_oai_ran_user() {
    ...
    local gnb_chart="$RAN_DIR/oai-gnb"
    local ue_chart="$RAN_DIR/oai-nr-ue"

    info "Deploying OAI gNB/NR-UE for UE $u on slice $s"

    sed -i "s/^name: .*/name: oai-gnb$u/" "$gnb_chart/Chart.yaml"
    sed -i -E "s|^([[:space:]]*)name: \"oai-gnb.*-sa\"|\\1name: \"oai-gnb$u-sa\"|" "$gnb_chart/values.yaml"
    set_yaml_value "$gnb_chart/values.yaml" mcc "\"208\""
    set_yaml_value "$gnb_chart/values.yaml" mnc "\"95\""
    set_yaml_value "$gnb_chart/values.yaml" mncLength "\"2\""
    set_yaml_value "$gnb_chart/values.yaml" tac "\"0x00a000\""
    set_yaml_value "$gnb_chart/values.yaml" nssaiSst "\"2$s\""
    set_yaml_value "$gnb_chart/values.yaml" nssaiSd0 "\"123\""
    set_yaml_value "$gnb_chart/values.yaml" amfIpAddress "\"$AMF_IP\""
    set_yaml_value "$gnb_chart/values.yaml" rfSimulator "\"server\""
    set_yaml_value "$gnb_chart/values.yaml" gnbName "\"oai-gnb$u-rfsim\""
    sed -i "/nodeSelector:/,/nodeName:/c\nodeSelector:\n  deplocation: $RAN_LOC\n\nnodeName: " "$gnb_chart/values.yaml"
    helm upgrade --install "gnb$u" "$gnb_chart" -n "$NAMESPACE"
    wait_for_pod "$NAMESPACE" "oai-gnb$u" 420
    ...
    set_yaml_value "$ue_chart/values.yaml" rfSimulator "\"oai-gnb$u\""
